...
>   above that list). Items 2 and 3 can both be oracle-tested today, and item 2
...
| `tests/` | 🟡 | Not mirrored. gofbp asserts against a generated 23,748-case sweep instead (`testdata/gen_cffdrs_reference.R`). Upstream's own test cases are still worth reading for edge cases the sweep does not reach — but note R does not install `tests/`, so they are not readable from the pinned container and need the upstream tree. What *is* installed and readable is `inst/extdata` (see below), which pairs with `data/test_fbp` as upstream's own FBP regression case. |
...
> |---|---|---|
> | 1 → 2 (CBH/CFL before C6) | **no** | C6's three functions take `ISI, FMC, RSS, RSO, CFB` — no table |
> | 1 → 3 (CBH/CFL before a complete TFC) | **no** | `total_fuel_consumption` takes CFL as a *parameter*; `fbp()` substitutes the table before the call |
> | 3 → 3 (TFC before HFI) | yes | `HFI <- fire_intensity(TFC, ROS)` |
> | everything → 5 (driver last) | yes | by definition |
> | 4 | independent | acceleration blocked by the sweep only. FMC was the other one; it was item 1 and went ✅ on 2026-09-18 |
>
> So the sentence above — "doing them out of order means writing code that cannot
> be oracle-tested yet" — is **false for items 2 and 3.** Both can be oracle-tested
> today, and item 2 against the fixture that already exists. What remains is a cost
> and design order, not a correctness one.
>
> **The list is deliberately left in its current order anyway.** Item 3's own note
...
> revisiting both later to lift its exclusion. Item 1's "ask before choosing" still
...
2. **C6 RSC** (`C6calc.r`) — removes the exclusion that six oracle tests carry
...
3. **TFC, HFI** (`total_fuel_consumption.r`, `fire_intensity.r`) — SFC landed on
   ...
   not blocked on item 1 either, and the claim that it was is withdrawn:**
