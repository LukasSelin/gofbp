...
//
// This is deliberately NOT part of the risk-index composite. The composite
// (riskindex/riskmodel) is a unitless FWI-scaled score whose range is tied to
// brandriskklass; FBP produces a rate of spread in **metres per minute**, a
// physical quantity. Multiplying them would destroy both: the composite would
// drift off the SMHI danger scale that eldningsförbud and the guidance page
// reference, and the spread rate would lose the units that make it verifiable
// against observed fire behaviour. They answer different questions —
// "how dangerous are conditions today" versus "if it starts here, how fast does
// it move" — so they are served as two separate overlays.
//
// Inputs are the same ones already resolved per request: ISI and BUI from SMHI
// (riskmodel.Features), a fuel type chosen by the caller, and slope, aspect,
// wind and FFMC from the terrain layer and the weather scalars.
//
// # Nothing Swedish lives here
//
...
// for Sweden, and no judgement about what Swedish ground is made of. That is a
// deliberate constraint, not an accident of layout — the claim "we run the
// Canadian FBP System, unmodified" is only checkable if the package making it
// contains nothing else, and it is the claim the product rests on.
//
// Every Swedish decision — which fuel type a stand maps to, what curing to assume,
// what to do with mire — lives in internal/fuelmap, which supplies the fuel code
// this package's functions take as an argument. If you find yourself adding a
// coefficient, a threshold or a default here because of something about Sweden,
...
// (EquivalentWind / NetEffectiveWind, giving WSE/WSV/RAZ), and ROS.
//
...
// alignment, the largest known per-pixel feature gap (see
// docs/analysis/spread-severity-plan.md §5).
//
...
// branch, which had our fully-cured grass spreading 2.2x too fast, and it is what
// caught the FFMC moisture constant being the rounded 147.2 rather than the exact
// 250·59.5/101 — see ffmcMoistureScale.
//
...
// It falls back to a local Rscript when one has cffdrs installed.
package fbp
...
// internal/surface.Spread is that caller.
...
// Nothing here computed ISI before the back-solve landed (it came from SMHI), so
...
// is an ordinary August afternoon on a steep Swedish esker. Without the floor the
// term goes negative, math.Log returns NaN, and that NaN reaches the served grid
// where NaN means "outside the parcel" — a hole in the overlay exactly where the
// risk is highest. TestEquivalentWindIsNonNegativeAndFiniteEverywhere is the
...
//
// Wind is km/h because ST-X-3 is. Converting Sweden's m/s is the caller's job and
// lives in internal/surface: this package speaks the published system's units and
// nothing else.
//
...
// negative and m^5.31 is NaN, and a NaN escaping here would reach the served grid
// as a hole meaning "outside the parcel".
